...
categories:
  - Java
createTime: 2021/11/18 01:00:09
permalink: /article/cfxx9lqp/
---
...
- 进入 nacos [下载](https://github.com/alibaba/nacos/releases) 页面，找到最新的 Assets 中的 zip 文件，例如：[nacos-server-2.0.3.zip](https://github.com/alibaba/nacos/releases/download/2.0.3/nacos-server-2.0.3.zip)，下载完成之后解压；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_1_01.webp)

- 然后可参考 Nacos [文档](https://nacos.io/zh-cn/docs/quick-start.html) 中的方式启动。注意，启动脚本中默认是集群模式，你可以在启动命令中显示指定模式，但为了使用方便建议修改默认为单机模式。这里以 Windows 脚本为例，打开刚刚解压好的文件夹，进入到 nacos/bin 目录下，找到 startup.cmd 文件；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_1_02.webp)

- 接着点右键编辑，打开搜索 `set MODE`，将`set MODE="cluster"`修改为`set MODE="standalone"`，然后保存关闭；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_1_03.webp)

- 双击 startup.cmd ，此时会打开一个 Dos 窗口，等到窗口显示`"Nacos started successfully in stand alone mode. use embedded storage`，即表示启动成功；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_1_04.webp)

- 接着打开浏览器输入：[http://127.0.0.1:8848/nacos](http://127.0.0.1:8848/nacos ) 即可看到登录页，用户名和密码均为 nacos，点击 **提交** 后进入主页；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_1_05.webp)

此时已完成 Nacos 安装和启动，接下来进行网关工程搭建。

## 2. 工程创建及路由分发

- 这里可参考官方文档，如果你搭建 Spring 项目见 [**Nacos Spring**](https://nacos.io/zh-cn/docs/quick-start-spring.html) ，如果你搭建 Spring Boot 项目见 [**Nacos Spring Boot**](https://nacos.io/zh-cn/docs/quick-start-spring-boot.html) ，如果你搭建 Spring Cloud 项目见  [**Nacos Spring Cloud**](https://nacos.io/zh-cn/docs/quick-start-spring-cloud.html) 。注意，Gateway 使用的是 WebFlux，是一个非阻塞异步的框架，和 SpringMvc 框架不同，因此不可以在网关工程的 pom.xml 文件中引入 `spring-webmvc` 的依赖。

- Nacos 有默认保留的命名空间 `public` ，你可以使用这个命名空间。我们这里新建一个命名空间 `dev_java` ；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_2_01.webp)

- 连接 nacos 配置需要写在 `bootstrap.yml` 中，启动项目时 `bootstrap.yml` 优先被加载，之后 `application.yml` 被加载；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_2_02.webp)

- 注意，`pom.xml` 文件中需要引入 `spring-cloud-starter-bootstrap` 依赖才可以支持加载 `bootstrap.yml` 文件。这里我们给网关服务命名为：example-gateway；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_2_03.webp)
   
- 接着在 Nacos 上新增配置文件；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_2_04.webp)

- 注意 **Data ID** 命名中有 .yaml 后缀，配置格式选择 YAML ，因为内容不可以为空，这里我们随意写一个符合 yaml 语法的配置，稍后再进行修改，然后点击 `发布`；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_2_05.webp)
...
![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_2_06.webp)
   
- 上述步骤完成后，网关工程搭建完成。接着再搭建两个普通的 SpringBoot 工程：`example-service-a`、`example-service-b`；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_2_07.webp)

- 然后启动这三个服务；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_2_08.webp)
   
- 用 Postman 请求一下，可以看到正常响应；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_2_09.webp)
...
![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_3.6_01.webp)
...
![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_4_01.webp)

- 在 Nacos 上创建 `example-gateway-router.json` 文件，用来存放路由配置；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_4_02.webp)

- 将网关工程中的 resources/router/rouerList.json 内容拷贝至 `example-gateway-router.json` 文件 中;

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_4_03.webp)

- 在 Nacos 上的 `example-gateway.yaml` 文件中配置如下，以启用动态路由，并指定路由配置文件；

![](https://cdn.jsdelivr.net/gh/Ma-yeah/picx-images-hosting@master/2021-11-18/01_4_04.webp)

- 要想实现动态路由效果，需要实现 `RouteDefinitionRepository`和 `ApplicationEventPublisherAware` 这两个接口；
...
在网关 `pom.xml` 文件中增加 `spring-cloud-starter-loadbalancer` 依赖，使其支持 `lb://` 的负载均衡协议。
...
---
Tip：本文完整示例代码已上传至 [GitHub](https://github.com/Blogeee/blog-java/tree/main/_20211010)
