...
categories:
  - Java
createTime: 2021/12/09 23:59:14
permalink: /article/prt57vpz/
---
...
#### 3.1.2. 创建 Handler Method `get` 和 `post`
...
#### 3.6.1. `User` 模型中新增属性 `learning` 和 `working`
...
#### 3.6.2. 定义两个校验组 `ValidLearningGroup` 和 `ValidWorkingGroup`
...
#### 3.7.2. 新增两个分组 `EmailSeq` 和 `PhoneSeq`
...
#### 3.7.4. 指定模型 `User` 中的 `email` 和 `phone` 属性的校验分组，并给 Handler Method `sequence` 增加 `UserSequence` 校验组
...
#### 4.1.1. 定义一个父类 `Person`，两个子类 `Teacher` 和 `Student`
...
---
Tip：本文完整示例代码已上传至 [GitHub](https://github.com/Blogeee/blog-java/tree/main/_20211209)